...
def get_synced_files():
    """Returns list of already synced HTML files to avoid redundant uploads."""
    synced = []
    headers = {"Authorization": f"token {GH_TOKEN}"}
    for folder in ["Insider", "episodes"]:
        r = requests.get(f"https://api.github.com/repos/{GH_REPO}/contents/{folder}", headers=headers)
        if r.status_code == 200:
            synced.extend([f['name'].replace('.html', '') for f in r.json() if f['name'].endswith('.html')])
    return synced

def clean_title_ai(raw):
    """Uses Gemini to extract professional Title and Year from messy Abyss filenames."""
    ...
    try:
        ...
        payload = {
            "contents": [{"parts": [{"text": f"Filename: '{raw}'. Extract ONLY the official Title and Year. Return as JSON: {{\"t\":\"Title\", \"y\":\"Year\"}}. No noise like .mp4 or 1080p."}]}],
    ...
    try:
        ...
        if res.get('results'):
            ...
            return {
                ...
                "back": f"https://image.tmdb.org/t/p/original{back}" if back else "",
...
def main():
    ...
    for item in items:
        ...
        # Add to global catalog for index rebuilding
        catalog.append({"n": name, "y": year, "u": path, "i": data['poster'], "t": "Movie" if not is_dir else "Series", "s": data['score']})

        # Skip already created pages to save time
        if iid in synced_ids: continue
        
        if not is_dir:
            # MOVIE PLAYER (Android Optimized & Clean Filtered)
    ...
    input{{width:100%;padding:14px 20px;border-radius:15px;border:1px solid #333;background:rgba(255,255,255,0.05);color:#fff;outline:none;font-size:16px;box-sizing:border-box;}}
    ...
    .m-card{{background:var(--card);border-radius:22px;overflow:hidden;border:1px solid #1a1a25;transition:0.3s;height:100%;box-shadow:0 15px 40px rgba(0,0,0,0.5);display:flex;flex-direction:column;}}
    ...
    .badge{{position:absolute;top:10px;left:10px;background:rgba(0,0,0,0.7);backdrop-filter:blur(5px);padding:5px 10px;border-radius:10px;font-size:0.7rem;font-weight:bold;color:var(--accent);border:1px solid rgba(0,210,255,0.2);}}
    .type{{position:absolute;bottom:12px;right:12px;background:var(--accent);color:#000;padding:3px 10px;border-radius:8px;font-size:0.6rem;font-weight:900;text-transform:uppercase;}}
    ...
    push_github("index.html", index_html, "Elite Total Refresh: Clean Titles & Fixed Android UI")
